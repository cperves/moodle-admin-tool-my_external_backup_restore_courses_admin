...
require_once ($CFG->dirroot.'/'.$CFG->admin.'/tool/my_external_backup_restore_courses_admin/locallib.php');
 ...
 */
class tool_my_external_backup_restore_course_admin_table extends table_sql implements renderable {
    /** @var int $perpage */
    private $perpage = 10;
    ...
    function __construct($perpage=null, $page=null, $rowoffset=0) {
        ...
        $this->define_baseurl(new moodle_url('/'.$CFG->admin.'/tool/my_external_backup_restore_courses_admin/managment.php'));

        $this->anyentries = tool_my_external_backup_restore_course_admin_any_entries();
        ...
        if ($rowoffset) {
 		...
 		$fields='*';
 		$from = '{block_external_backuprestore}';
 		$where = 'true';
